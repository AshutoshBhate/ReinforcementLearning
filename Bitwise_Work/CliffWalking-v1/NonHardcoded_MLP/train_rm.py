...
def train_reward_model(reward_model, pairs, teacher, optimizer):
    ...
    for segment_1, segment_2 in pairs:
        ...
        # Handle Tie: Target becomes 0.5 (Neutral)
        if label == -1:
            target_val = 0.5
        else:
        ...
        label_tensor = torch.tensor(
            [target_val], dtype=torch.float32, device=config.DEVICE
        )
        ...
        s1_act_one_hot = torch.zeros((len(s1_act), config.ACTION_DIM), device=config.DEVICE)
        ...
        s2_act_one_hot = torch.zeros((len(s2_act), config.ACTION_DIM), device=config.DEVICE)
    ...
    if valid_pairs_count > 0:
        batch_loss = batch_loss / valid_pairs_count
        batch_loss.backward()
        optimizer.step()
